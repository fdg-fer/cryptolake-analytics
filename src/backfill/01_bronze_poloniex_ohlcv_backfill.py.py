...
# MAGIC %md
# MAGIC # Bronze Backfill: Poloniex OHLCV Historical Data
# MAGIC
# MAGIC ## Objective
# MAGIC
# MAGIC Backfill historical hourly OHLCV candles from Poloniex for BTC, ETH, SOL, ADA, and LINK.
# MAGIC
# MAGIC ## Date Range
# MAGIC
...
# DBTITLE 1,API Functions - Poloniex
def fetch_poloniex_candles(symbol, start_time_ms, end_time_ms, limit=MAX_LIMIT):
    ...
    with urllib.request.urlopen(url, timeout=30) as response:
        if response.status != 200:
            raise RuntimeError(f"Poloniex API error: {response.status}")
    ...
    for candle in data:
        candles.append({
            "symbol": symbol,
            "open_time_ms": int(candle[12]),
            "open_time": datetime.fromtimestamp(int(candle[12]) / 1000, tz=timezone.utc),
            "close_time_ms": int(candle[13]),
            "close_time": datetime.fromtimestamp(int(candle[13]) / 1000, tz=timezone.utc),
            "open": candle[2],
            "high": candle[1],
            "low": candle[0],
            "close": candle[3],
            "volume": candle[5],
            "quote_asset_volume": candle[4],
            "trade_count": int(candle[8]),
            "taker_buy_base_volume": candle[7],
            "taker_buy_quote_volume": candle[6],
            "interval": INTERVAL,
        })
...
def collect_hourly_candles(symbols, target_date):
    ...
    for symbol in symbols:
        try:
            candles = fetch_poloniex_candles(symbol, start_time_ms, end_time_ms)
...
# Align schema with existing table (drop extra columns)
df_backfill_aligned = df_backfill.select(
    "close",
    "close_time_ms",
    "high",
    "interval",
