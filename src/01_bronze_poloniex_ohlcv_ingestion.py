...
# MAGIC %md
# MAGIC # Camada Bronze: Ingestão de Dados Poloniex OHLCV
# MAGIC
# MAGIC ## Objetivo
# MAGIC
# MAGIC Ingerir candles horários de OHLCV da API pública da Poloniex para BTC, ETH, SOL, ADA e LINK.
# MAGIC
...
# MAGIC
# MAGIC **Destino:** `uc_sa_br_dev.bronze_poloniex_ohlcv.hourly`
# MAGIC
...
# MAGIC
# MAGIC - **Campos da API:** symbol, low, high, open, close, volume, start_time_ms, close_time_ms, interval, trade_count (tipos originais)
# MAGIC - **Metadata de Auditoria:** ingested_at (timestamp do processo)
# MAGIC - **Partição Física:** rate_date (derivado de start_time_ms)
# MAGIC - **Fonte:** Implícita no nome do schema (bronze_poloniex_ohlcv)
# MAGIC
...
# DBTITLE 1,Funções utilitárias de timestamp
def utc_now_ms():
...
# DBTITLE 1,Funções de API Poloniex
...
# DBTITLE 1,Buscar dados da Poloniex
...
# MAGIC ```python
# MAGIC # Na célula Configuration, alterar TARGET_DATE:
# MAGIC TARGET_DATE = date(2026, 6, 20)  # Reprocessar 2026-06-20
# MAGIC
# MAGIC # Executar células 9-11
...
# MAGIC         .withColumn("rate_date", F.to_date(F.from_unixtime(F.col("start_time_ms") / 1000)))
# MAGIC     )
# MAGIC     bronze_poloniex.write_table(
# MAGIC         df, TABLE_NAME, 
...
# MAGIC df_dates = (
# MAGIC     bronze_poloniex.read_table(TABLE_NAME)
